...
} from '\var\task\B3TR BEACH\b3tr-beach-next/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer } from '\var\task\B3TR BEACH\b3tr-beach-next/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\B3TR BEACH\b3tr-beach-next')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return tracer.withActiveSpan('Next.js Server Handler', async (span) => {
      span.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\B3TR BEACH\b3tr-beach-next',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\B3TR BEACH\b3tr-beach-next\.netlify\dist\run\handlers\server.js')
